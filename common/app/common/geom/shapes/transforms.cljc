...
(defn transform-content [content transform]
  (let [set-tr (fn [param px py]
                 (let [tr-point (-> (gpt/point (get param px) (get param py))
        ...
        (fn [{:keys [x y c1x c1y c2x c2y] :as params}]
          (cond-> params
            ...
            (not (nil? c2x)) (set-tr :c1x :c1y)))]

    (mapv #(update % :params transform-params) content)))

(defn apply-transform-path
  [shape transform]
  (let [content (transform-content (:content shape) transform)
